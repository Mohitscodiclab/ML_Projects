...
def hash_row(row):
    """Convert a CSV row to a SHA-256 hash string"""
    row_str = ",".join(row)
    return hashlib.sha256(row_str.encode()).hexdigest()

file = "votes.csv"
backup_file = "data/votes_backup.csv"

# Step 1: Create a backup copy
shutil.copy(file, backup_file)
print(f"📂 Backup saved as '{backup_file}'")

# Step 2: Read, hash, and overwrite the original file
with open(file, "r", newline="") as infile:
    reader = csv.reader(infile)
    hashed_rows = [[hash_row(row)] for row in reader]  # replace each row with hash

with open(file, "w", newline="") as outfile:
    writer = csv.writer(outfile)
    writer.writerows(hashed_rows)

print(f"✅ Original file '{file}' has been replaced with hashed values.")
